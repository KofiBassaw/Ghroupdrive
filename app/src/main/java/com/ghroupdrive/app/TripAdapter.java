...
public class TripAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public TripAdapter(ArrayList<GettersAndSetters> itemList, Context c) {
        mItemList = itemList;
        this._c = c;
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if(viewType == StaticVariables.TOPTYPE){
            final View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.trip_top_item, parent, false);
            return TripTopViewHolder.newInstance(view);
        }else if(viewType == StaticVariables.CONTENT)
        {
            final View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.trip_content, parent, false);
